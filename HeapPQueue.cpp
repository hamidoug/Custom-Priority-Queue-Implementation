...
#include <error.h>
using namespace std;
/* This set of functions creates a Heap Priority Queue that enqueues DataPoints with associated weights where each child
 * has a higher weight than its parent */

/* initiate variables in private section of header and set array that elems is pointing to to have allocatedSize */
...
/* deletes newly created arrays to prevent memory leaks */
HeapPQueue::~HeapPQueue() {
    delete[] elems;
}

/* gives the size of the array (ie, how many dataPoints are in it) */
int HeapPQueue::size() const {
    return logicalSize; //not allocatedSize because logicalSize is the number of how many DataPoints are inside and allocatedSize is available sllots in the array
}
/* checks if there are no DataPoints in the array */
bool HeapPQueue::isEmpty() const {
    return size() == 0; //uses the size function previously written because that returns logical size
}

/* enqueues new dataPoints into array and bubbles up until all children weights are greater than their parents' */
void HeapPQueue::enqueue(const DataPoint& data) {
    if (allocatedSize == logicalSize + 1) {
        grow(); //calls function to increase allocatedSize if there is no space to enqueue another element
    }
    int num = logicalSize + 1; //new dataPoint has been enqueued
    elems[num] = data;
    while (elems[num / 2].weight > elems[num].weight && num > 1) {
        swap(elems[num / 2], elems[num]); //swapping if parent weight is greater than child weight
        num = num / 2;
    }
    logicalSize++;
}

/* Function to double the allocatedSize of the array to allow for more dataPoints to be enqueued when there is no more space to do so */
void HeapPQueue::grow() {
    allocatedSize *= 2;
    DataPoint* helper = new DataPoint[allocatedSize];
    for (int i = 0; i < logicalSize + 1; i++) {
        helper[i] = elems[i]; //copy over all already inputted dataPoints to helper array
    }
    delete[] elems; //prevent memory leak
    elems = helper;
}

/* Returns the dataPoint with the lowest weight in the array */
DataPoint HeapPQueue::peek() const {
    if (logicalSize == 0) {
        error("Queue is empty"); //cannot return a dataPoint if there isn't one
    }
    return elems[1]; //the dataPoint with the lowest weight is always the first one
}

/* Dequeues dataPoints from array by switching the last and first one and then bubbling down to make sure all parent weights
 * are less than their children's weights */
DataPoint HeapPQueue::dequeue() {
    if (logicalSize == 0) {
        error("Queue is empty, cannot dequeue from empty queue");
    }
    DataPoint result = peek();
    swap(elems[1], elems[logicalSize]);
    logicalSize--;
    int indextobubledown = 1;
    while (2 * indextobubledown <= logicalSize && elems[2 * indextobubledown].weight < elems[indextobubledown].weight
           || (2 * indextobubledown) + 1 <= logicalSize && elems[(2 * indextobubledown) + 1].weight < elems[indextobubledown].weight) { //repeatedly bubbles down while a child's weight is greater than the parent's
        if (2 * indextobubledown <= logicalSize && elems[2 * indextobubledown].weight < elems[indextobubledown].weight
                && (2 * indextobubledown) + 1 <= logicalSize && elems[(2 * indextobubledown) + 1].weight < elems[indextobubledown].weight) {
            if (elems[2 * indextobubledown].weight < elems[(2 * indextobubledown) + 1].weight){ //check if needs to swap with the left child because it has the smaller weight out of the children (and both children have a smaller weight than the parent)
                swap(elems[2 * indextobubledown], elems[indextobubledown]);
                indextobubledown = 2 * indextobubledown;
            }
            else {
                swap(elems[(2 * indextobubledown) + 1], elems[indextobubledown]); //checks if needs to swap with the right child or can pick arbitrarily because of a tie (picks right in this case) (and both children have a smaller weight than the parent)
                indextobubledown = (2 * indextobubledown) + 1;
            }
        }
        else if (2 * indextobubledown <= logicalSize && elems[2 * indextobubledown].weight < elems[indextobubledown].weight ) { //swaps if only the left child has a lower weight
            swap(elems[2 * indextobubledown], elems[indextobubledown]);
            indextobubledown = 2 * indextobubledown;
        }
        else if ((2 * indextobubledown) + 1 <= logicalSize && elems[(2 * indextobubledown) + 1].weight < elems[indextobubledown].weight) { //swaps if only the right child has a lower weight
            swap(elems[(2 * indextobubledown) + 1], elems[indextobubledown]);
            indextobubledown = (2 * indextobubledown) + 1;
        }
    }
    return result;
...
STUDENT_TEST("Breaks ties and bubbles down to the right.") {
    HeapPQueue pq;
